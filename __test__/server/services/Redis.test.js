...
const { getKey, setKey, setWithExpire, delKey } = require('../../../server/services/Redis');
...
describe('Redis', () => {
  ...
  describe('delKey', () => {
    it('should delete the key', async () => {
      ...
      const result = await delKey('sampleKey');
      expect(result).toEqual(1);
      expect(redisMock.del).toHaveBeenCalledWith('sampleKey');
    });

    it('should return null on error', async () => {
      const redisMock = new Redis();
      redisMock.del.mockRejectedValue(new Error('Mock error'));
      const result = await delKey('sampleKey');
      expect(result).toEqual(null);
      expect(redisMock.del).toHaveBeenCalledWith('sampleKey');
    });
